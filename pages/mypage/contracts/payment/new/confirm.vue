      ...
      <div class="form-group mb-41">
        <div class="information-table table--double">
          ...
          <div class="information-table__row">
            ...
            <div class="info-table__content">
              {{ dataCustomerInfo.birthDate?.replaceAll('-', '/') }}
            </div>
          </div>
          <div class="information-table__row">
            <div class="info-table__label">
        ...
        <div class="headline-change">
          ...
          <div class="headline-change__change-link">
            <nuxt-link
              ...
              class="tracking"
              :to="{ path: '/mypage/contracts/payment/new/' }"
            >
              {{ $t('common.change') }}
            </nuxt-link>
          </div>
  ...
  beforeRouteEnter(to, from, next) {
    ...
    if (checkReload) {
      return next((vm) => {
        ...
        vm.$router.push('/mypage/contracts/payment/new/');
      });
    }

    if (from.name !== 'mypage-contracts-payment-new' && !checkReload) {
      return next((vm) => {
